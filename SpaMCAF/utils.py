...
def generation_coord_10x(
    ...
    fill_coor_df = pd.concat([coor_df, coor_df_1, coor_df_2, coor_df_3, coor_df_4, coor_df_5, coor_df_6, coor_df_7])  # , coor_df_4, coor_df_5, coor_df_6, coor_df_7
    # fill_coor_df = fill_coor_df.drop_duplicates(subset=fill_coor_df.columns)  # 删除重复行

    fill_spatial = pd.concat(
        [fill_spatial_ori, fill_spatial_1, fill_spatial_2, fill_spatial_3, fill_spatial_4, fill_spatial_5, fill_spatial_6, fill_spatial_7])  # , fill_spatial_4, fill_spatial_5, fill_spatial_6, fill_spatial_7
...
def generation_feature_10x(image_path, ori_spatial, fill_spatial):
    ...
    sample_features_672 = UNI_features(img_path=image_path, spatial=ori_spatial, patch_size=672)


    fill_features_224 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=224)
    fill_features_672 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=672)



    sample_features = {"sample_features_224": sample_features_224,  "sample_features_672": sample_features_672}
    fill_features = {"fill_features_224": fill_features_224,
                     "fill_features_672": fill_features_672}
...
def recovery_coord(
    ...
    """ This function generates spatial location for Spatial Transcriptomics data.
        Args:
            ...
            path1: Path to save the output files.
        Return:
            coor_df: Spatial location of down-sampled data.
            fill_coor_df: Spatial location of recovered data.
            sample_index: Index of down-sampled data.
            sample_barcode: Barcode of down-sampled data.
            del_index: Index of data that is not selected.
            del_barcode: Barcode of data that is not selected.
    """
    # 获取原始空间坐标数据
    ...
    spot_coord_del = spot_coord_ori.iloc[del_index]
    image_spatial_del = image_spatial_ori.iloc[del_index]
    del_barcode = spot_coord_ori.index[del_index]
    ...
    np.savetxt(path1 + "/sample_barcode.txt", sample_barcode, fmt='%s')
    np.savetxt(path1 + "/del_barcode.txt", del_barcode, fmt='%s')

    # 返回数据
    return spot_coord_sample, spot_coord_ori, image_spatial_sample, image_spatial_ori, sample_index, sample_barcode, del_index, del_barcode
...
def get_sectionData(section_id):
    ...
    if section_id in section_list:
        input_dir = rf'D:/HisToSGE/Data/DLPFC/{section_id}'
    ...
    if section_id == "FFPE":
        input_dir = 'D:/HisToSGE/Data/BreastCancer2'
        adata = sc.read_visium(path=input_dir,
    ...
    if section_id == "MBA":
        input_dir = 'D:/HisToSGE/Data/MBA'
        adata = sc.read_visium(path=input_dir,
                               count_file='filtered_feature_bc_matrix.h5')
        adata.var_names_make_unique()
        image_path = input_dir + "/151507_full_image.tif"
...
def get_trainData(adata, image_path, experiment, data_type, path1, coord_sf=77):
    if experiment == 'generation' and data_type == '10x':
        ...
        # 解包时只获取前6个返回值
        spot_coord_sample, spot_coord_ori, image_spatial_sample, image_spatial_ori, sample_index, sample_barcode, _, _ = recovery_coord(
            adata,
            down_ratio=0.5,
            path1=path1
        )
