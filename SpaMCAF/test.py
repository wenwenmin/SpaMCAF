...
adata_SpaMCAF = sc.read_h5ad(save_model_path + '/recovered_data.h5ad')
print(adata_SpaMCAF)
adata_sample = sc.read_h5ad(save_model_path + '/sampled_data.h5ad')
pr_stage = np.zeros(adata_SpaMCAF.shape[1])
P_value = np.ones(adata_SpaMCAF.shape[1])
mse_values = np.zeros(adata_SpaMCAF.shape[1])
mae_values = np.zeros(adata_SpaMCAF.shape[1])
used_gene = adata_SpaMCAF.var.index

for it in tqdm(range(adata_SpaMCAF.shape[1])):
    pr_stage[it], P_value[it] = \
        pearsonr(adata_SpaMCAF[:, used_gene[it]].X.toarray().squeeze(), adata[:, used_gene[it]].X.toarray().squeeze())
    mse_values[it] = mean_squared_error(adata_SpaMCAF[:, used_gene[it]].X.toarray().squeeze(),
                                            adata[:, used_gene[it]].X.toarray().squeeze())
    mae_values[it] = mean_absolute_error(adata_SpaMCAF[:, used_gene[it]].X.toarray().squeeze(),
                                             adata[:, used_gene[it]].X.toarray().squeeze())
...
print("section_id:", section_id, "AVG MAE:", np.mean(mae_values))
    ...
    # break
# print(adata_SpaMCAF, adata_sample)
...
# adata_SpaMCAF.obsm["spatial"] = adata.obsm["spatial"]
# #使用 KMeans 进行聚类
# ARI_SpaMCAF = {}
# ARI_h = {}
#
...
# kmeans_labels = kmeans.fit_predict(adata_SpaMCAF.X) # h_features
#
# adata_SpaMCAF.obs['kmeans'] = kmeans_labels.astype(str)
#
# ari = adjusted_rand_score(adata.obs['layer'].astype(str), adata_SpaMCAF.obs['kmeans'].astype(str))
# # if ari < 0.18:
# sc.pl.spatial(adata_SpaMCAF, color='kmeans', title='KMeans Clustering', spot_size=150, show=False)
...
#     # plt.savefig(rf'C:\Users\DELL\Desktop\学术垃圾\SpaMCAF/{section_id}_{ari}.pdf', format='pdf')
#     ARI_h[i] = ari
#     print(i, ari)
#     break
#
#
# max_i = max(ARI_SpaMCAF, key=ARI_SpaMCAF.get)
# max_ari = ARI_SpaMCAF[max_i]
# print(f"Max ARI_SpaMCAF: {max_ari} at PCA components: {max_i}")
#
...
#     sc.pp.pca(adata_SpaMCAF, n_comps=100)
#     sc.tl.tsne(adata_SpaMCAF)
#     # print(adata_SpaMCAF)
#     adata_SpaMCAF.obsm["spatial"] = adata.obsm["spatial"]
#     kmeans_adata_stage = KMeans(n_clusters=7, init='k-means++', random_state=0).fit(adata_SpaMCAF.obsm["X_pca"])
#     adata_SpaMCAF.obs['kmeans'] = kmeans_adata_stage.labels_.astype(str)
#     ari = adjusted_rand_score(adata.obs['layer'].astype(str), adata_SpaMCAF.obs['kmeans'].astype(str))
#     sc.pl.spatial(adata_SpaMCAF, basis="spatial", color='kmeans', title=f'KMeans+{ari}', spot_size=150)
#     ARI[i] = ari
#     break
# # 找出最大ARI值及其对应的i
...
#
#
# pcc_show_gene = {gene: pearsonr(adata_SpaMCAF[:, gene].X.toarray().squeeze(),
#                                 adata[:, gene].X.toarray().squeeze())[0]
#                  for gene in show_gene}
#
# # %%
# sc.set_figure_params(dpi=80, figsize=(2.8, 3))
# sc.pl.embedding(adata_SpaMCAF, basis="coord", color=show_gene, s=30, show=True)
# ax = plt.gca()
# for spine in ax.spines.values():
#     spine.set_edgecolor('black')
#     spine.set_linewidth(1)
...
# sc.set_figure_params(dpi=80, figsize=(2.8, 3))
# sc.pl.embedding(adata_SpaMCAF, basis="coord", color=show_gene, s=30, show=False)
# ax = plt.gca()
# for spine in ax.spines.values():
#     spine.set_edgecolor('black')
#     spine.set_linewidth(1)
