...
def train(section_id, experiment, data_type, save_model_path, path1):
    ...
    model = TriHRGE(in_features=1024, n_genes=1000, depth=1, heads=4, dropout=0.)
    model.train()
    ...
    with tqdm(range(train_epoch), total=train_epoch, desc='Epochs') as epoch:
        for j in epoch:
            ...
            for exp, pos, image in dataloader_loader:
                ...
                if xrecon is None:
                    raise ValueError("xrecon cannot be None")
                recon_loss = mse_loss(xrecon,
                                      exp)  # + 0.1 * L1_loss(xrecon,exp)  # + 0.1 * sliced_wasserstein_distance(xrecon, exp, 1000, device=device)
                recon_loss.backward()
                model_optim.step()
                if j <= 50:
    ...
    if experiment == "generation":
        ...
        with torch.no_grad():
            for pos, image in tqdm(dataloader_test):
                ...
                if pred is None:
                    raise ValueError("pred cannot be None")
                if preds is None:
                    preds = pred.squeeze()
                else:
                    pred = pred.squeeze()
...
if __name__ == "__main__":
    ...
    for section_id in section_list:
        #section_id = "151507"  # mouse_brain Breast Cancer FFPE MBA
